...
TOKEN = 'OTU4MDYzNjYyNzgwNzg0Njgx.YkH4UQ.5S-qLpP3E5PPkbuj47IqkGDRLpY'
LIMIT =  50
def randomString(N):
...
@bot.event
async def on_ready():
   await bot.change_presence(status=discord.Status.online)# activity=discord.Activity(type=discord.ActivityType.watching, name="จะดูอะไรก็เรื่องของผมครับ cream"))
...
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.content.startswith('sms'))):
        if (phone == None or amount == None):
            embed=discord.Embed(title="ยิงเบอร์ไม่สำเร็จ", description="```#กรุณาใส่ข้อมูลให้ครบถ้วน#```", color=0x00ff00, timestamp=datetime.utcnow())
            embed.set_footer(text="PT3340")
            ...
            try:
                amount = int(amount)
                if (amount > LIMIT):
                    embed=discord.Embed(title="Unsuccess Spam", description=f"```#สามารถยิงได้รอบละ {LIMIT} ครั้ง```", color=0x00ff00, timestamp=datetime.utcnow())
                    embed.set_footer(text="PT3340")
                    ...
                    embed.set_image(url='')
                    await ctx.reply(embed=embed)
                elif phone == '0611873377':
                    embed=discord.Embed(title="Success Spam", description=f"```อย่ายิงเบอร์เจ้าของดิส ไอเวร```", color=0x00ff00, timestamp=datetime.utcnow())
                    embed.set_footer(text="PT3340")
                    embed.set_thumbnail(url='https://i.pinimg.com/originals/70/a5/52/70a552e8e955049c8587b2d7606cd6a6.gif')
                    embed.set_image(url='')
                    await ctx.reply(embed=embed)
                else:
                    embed=discord.Embed(title="PT3340", color=0x00ff00, timestamp=datetime.utcnow(),)
                    embed.set_footer(text="PT3340")
                 #   embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/934796371037405224/955980347458355200/92004028_p0.png')
                    embed.set_image(url='https://images-ext-2.discordapp.net/external/S5DoGqxs6x4q3ky-K89H42UEPAVjclkHJ_LvCPP9Wpk/https/media.discordapp.net/attachments/905106957822754876/915592008926638090/64b7e585d494a88c78bf65482e7efa01.gif')
                    embed.add_field(name="กำลังยิงไปที่เบอร์", value="``` 0********* ```", inline=False)
                    embed.add_field(name="ความเร็วเฉลี่ยสูงสุด", value="``` 135 ```", inline=False)
                    embed.add_field(name="ความเร็วเฉลี่ยต่ำสุด", value="``` 5 ```", inline=True)
                    embed.add_field(name="บอทมีปัญหาติดต่อได้ที่", value="``` !𝕻𝖎𝖞𝖆𝖜𝖆𝖙#9999  ```", inline=True)
                    components = ([
        [Button(label="Open At 8 April", style="3", emoji = "❤️", custom_id="the bet"), Button(label="Open At 8 April", style="4", emoji = "❤️", custom_id="BetaPremium"), Button(label="Open At 8 April", style="4", emoji = "❤️", custom_id="BetaDev")]
        ])
                    await ctx.reply(embed=embed)
                    await ctx.send("Offical Discord \n https://discord.gg/BkSuGB4NZ3")
                    await ctx.reply(components=components)
                    await ctx.send("คำสั่งเพิ่มเติม", components = [
        Select(
            placeholder = "กดเพื่อเลือกเมนู",
            options = [
                SelectOption(label="บอทดับแจ้งแอดมิน", value="Max"),
                SelectOption(label="บอทดับแจ้งแอดมิน", value="Vip"),
                SelectOption(label="บอทดับแจ้งแอดมิน", value="Premium"),
                SelectOption(label="บอทดับแจ้งแอดมิน", value="Partner")
            ]
        )
    ])
                    startall(phone, amount)
            except:
                embed=discord.Embed(title="Unsuccess Spam", description="```คุณใส่จำนวนไม่ถูกต้องหรือเกิดจากบัค \n ไม่ส่งผลต่อการยิง \n บอทยังคงยิงไปที่เบอร์ \n {phone} นะครับ```", color=0x00ff00, timestamp=datetime.utcnow())
                embed.set_footer(text="PT3340")
        ...
        embed=discord.Embed(title="Unsuccess Spam", description="```#โปรดใช้คำสั่งในห้อง | อิอิ```", color=0x00ff00, timestamp=datetime.utcnow())
        embed.set_footer(text="PT3340")
        embed.set_author(name='PT3340')
        embed.set_thumbnail(url='https://images-ext-2.discordapp.net/external/S5DoGqxs6x4q3ky-K89H42UEPAVjclkHJ_LvCPP9Wpk/https/media.discordapp.net/attachments/905106957822754876/915592008926638090/64b7e585d494a88c78bf65482e7efa01.gif')
        embed.set_image(url='')
        await ctx.reply(embed=embed)
